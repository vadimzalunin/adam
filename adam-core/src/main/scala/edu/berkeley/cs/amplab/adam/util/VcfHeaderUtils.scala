...
private[util] class VcfHeaderBuilder (samples: List[String]) {
  ...
  val headerLines: java.util.Set[VCFHeaderLine] = Set[VCFHeaderLine]()
  ...
  private def addStandardLines() {
    val keys = List(VCFConstants.GENOTYPE_KEY,
    ...
    VCFStandardHeaderLines.addStandardFormatLines(headerLines, false, keys)
  }
  ...
  def build(): VCFHeader = {
    ...
    val stdLines: Set[VCFHeaderLine] = headerLines
    val lines: Set[VCFHeaderLine] = contigLines.toSet ++ stdLines
