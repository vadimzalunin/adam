...
import edu.berkeley.cs.amplab.adam.avro.{ADAMFastaNucleotideContig, Base}
...
private[adam] object FastaConverter {
  ...
  /**
   * Converts an RDD containing ints and strings into an RDD containing ADAM FASTA data.
   *
   * @note Input dataset is assumed to have come in from a Hadoop text file reader.
   *
   ...
   */
  def apply (rdd: RDD[(Int, String)]): RDD[ADAMFastaNucleotideContig] = {
    val filtered = rdd.map(kv => (kv._1, kv._2.trim()))
      .filter((kv: (Int, String)) => !kv._2.startsWith(";"))

    val indices: Array[Int] = rdd.filter(kv => kv._2.startsWith(">"))
      .map(kv => kv._1)
      .collect()
      .sortWith(_ < _)
      .reverse

    val groupedContigs: RDD[(Int, Seq[(Int, String)])] = if (indices.length == 0) {
      filtered.keyBy(kv => -1)
        .groupByKey()
    } else {
      filtered.keyBy((kv: (Int, String)) => indices.find(_ <= kv._1))
        .filter((kv: (Option[Int], (Int, String))) => kv._1.isDefined)
    ...
    val convertedData = groupedContigs.map(kv => {
      ...
      val (name, comment): (Option[String], Option[String]) = if (descriptionLine.length == 0) {
